...
const bienvenidaScene = new Scenes.WizardScene('set_bienvenida',
  ...
  async (ctx) => {
    ...
    ctx.reply(`✅ Guardada! Vista previa:`); await ctx.reply(texto.replace('{nombre}', ctx.from.first_name), {entities: entidades,...menuPrincipal()});
...
bot.start(async (ctx) => {
  ...
  try{ const snap=await getDoc(doc(db,"config","bienvenida")); if(snap.exists()){ texto=snap.data().texto; entidades=snap.data().entidades||[]; } }catch{}
  await ctx.reply(texto.replace('{nombre}', ctx.from.first_name), {entities: entidades,...menuPrincipal()});
});
...
bot.command('ver_bienvenida', async (ctx) => {
  ...
  await ctx.reply(`Actual: ${snap.data().texto}`, {entities: snap.data().entidades,...menuPrincipal()});
});
...
// ACCIONES BOTONES INLINE (ESTO ES LO QUE SE TE HABÍA BORRADO)
...
bot.action('menu_chatbot', (ctx) => {
  ...
  ctx.reply('🤖 Panel Chatbot:', Markup.inlineKeyboard([
    ...
    [Markup.button.callback('✨ Cambiar Bienvenida','set_bien'), Markup.button.callback('⬅️ Principal','menu_principal')]
  ]));
});
...
bot.action(/^sendmod_/, async (ctx) => {
  ...
  const data=snap.data(); await ctx.reply(data.texto, {entities: data.entidades}); ctx.answerCbQuery();
});
bot.action('ver_bienvenida', async (ctx) => {
  ctx.answerCbQuery();
  const snap=await getDoc(doc(db,"config","bienvenida"));
  if(!snap.exists()) return ctx.reply('No hay bienvenida', menuPrincipal());
  await ctx.reply(snap.data().texto, {entities: snap.data().entidades,...menuPrincipal()});
});
...
bot.on('business_message', async (ctx) => {
  ...
  for(const d of snap.docs){
    if(textoUsuario.toLowerCase().includes(d.data().trigger.toLowerCase())){
      try{ await ctx.telegram.sendMessage(msg.chat.id, d.data().texto, {entities: d.data().entidades, business_connection_id: connId});
        await bot.telegram.sendMessage(LOG_GROUP_ID, `🤖 Auto-respuesta a ${userName}: ${d.data().texto}`, {message_thread_id: topicId});
      }catch(e){} return;
    }
  }
});

// --- RESPONDER DESDE EL GRUPO CON /r ---
bot.command('r', async (ctx) => {
  ...
  if(!userFound) return ctx.reply('No encontré usuario de este tema. Reenvía un mensaje de él primero.');

  const bizSnap=await getDocs(collection(db,"business"));
  if(bizSnap.empty) return ctx.reply('❌ No hay cuentas conectadas como chatbot. Reconecta en Ajustes > Telegram Business > Chatbots');
...
bot.on('text', async (ctx, next) => {
  ...
  if(String(ctx.chat.id)===String(LOG_GROUP_ID)) return; // no responder a logs
  const snap=await getDocs(collection(db,"respuestas"));
  for(const d of snap.docs){ if(ctx.message.text.toLowerCase().includes(d.data().trigger.toLowerCase())){ await ctx.reply(d.data().texto, {entities: d.data().entidades}).catch(()=>{}); break; } }
  return next();
});
...
bot.launch().then(()=> console.log('✅ V5.1 FIX BOTONES + TOPICS'));
const app=express(); app.get('/', (req,res)=>res.send('V5.1 ON')); app.listen(process.env.PORT||3000);
